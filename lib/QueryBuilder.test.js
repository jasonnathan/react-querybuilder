...
describe("<QueryBuilder />", () => {
  ...
  describe("when initial query is provided", () => {
    ...
    beforeEach(() => {
      ...
      dom = mount(<QueryBuilder fields={fields} query={query} />);
    });
    ...
    it("should have the Rule with the correct props", () => {
      ...
      expect(rule.props().field)
        .to
        .equal("firstName");
      expect(rule.props().value)
        .to
        .equal("Test");
      expect(rule.props().operator)
        .to
        .equal("=");
    });
    ...
    it("should have a select control with the provided fields", () => {
      ...
      expect(rule.find(".rule-fields option"))
        .to
        .have
        .length(3);
    });
    ...
    it("should have an field selector with the correct field", () => {
      ...
      expect(rule.find(".rule-fields").props().value)
        .to
        .equal("firstName");
    });
    ...
    it("should have an operator selector with the correct operator", () => {
      ...
      expect(rule.find(".rule-operators").props().value)
        .to
        .equal("=");
    });
    ...
    it("should have an input control with the correct value", () => {
      ...
      expect(rule.find("input").props().value)
        .to
        .equal("Test");
    });
  });
  ...
  describe("when initial operators are provided", () => {
    ...
    beforeEach(() => {
      ...
      dom = mount(
        <QueryBuilder fields={fields} operators={operators} query={query} />
      );
    });

    it("should use the given operators", () => {
      const operatorOptions = dom
        .find("Rule")
        .find(".rule-operators option");

      expect(operatorOptions.length)
        .to
        .equal(4);
    });

    it("should match the label of the first operator", () => {
      const operatorOption = dom
        .find("Rule")
        .find(".rule-operators option")
        .first();

      expect(operatorOption.text())
        .to
        .equal("Custom Is Null");
    });
  });
});
