...
describe("<RuleGroup />", () => {
  ...
  describe("combinator selector as <ValueSelector />", () => {
    ...
    it("should have options set to expected combinators", () => {
      ...
      schema.combinators = expectedCombinators;
      const dom = shallow(<RuleGroup {...props} />);

      expect(dom.find("ValueSelector").props().options)
        .to
        .equal(expectedCombinators);
    });

    it("should have the default selected value set to \"and\"", () => {
      const dom = shallow(<RuleGroup {...props} />);
      expect(dom.find("ValueSelector").props().value)
        .to
        .equal("and");
    });
    ...
    it("should have the onChange method handler", () => {
      const dom = shallow(<RuleGroup {...props} />);
      expect(dom.find("ValueSelector").props().handleOnChange)
        .to
        .be
        .a("function");
    });

    behavesLikeAnElementWithClassNames("ValueSelector", "ruleGroup-combinators", "custom-combinators-class");
  });
  ...
  describe("when 2 rules exist", () => {
    ...
    it("has the first rule with the correct values", () => {
      ...
      expect(ruleProps.id)
        .to
        .equal("rule_id_1");
      expect(ruleProps.field)
        .to
        .equal("field_1");
      expect(ruleProps.operator)
        .to
        .equal("operator_1");
      expect(ruleProps.value)
        .to
        .equal("value_1");
    });
  });
  ...
  describe("when 1 rule group exists", () => {
    ...
    it("has 1 <RuleGroup /> with expected properties", () => {
      const dom = shallow(<RuleGroup {...props} />);
      const groupProps = dom
        .find("RuleGroup")
        .props();
      expect(groupProps.id)
        .to
        .equal("rule_group_id_1");
      expect(groupProps.parentId)
        .to
        .equal("id");
      expect(groupProps.rules)
        .to
        .be
        .an("array");
      expect(groupProps.combinator)
        .to
        .equal("and");
    });
  });
  ...
  describe("#onCombinatorChange", () => {
    it("calls the #onPropChange from the schema with expected values", () => {
      ...
      expect(actualProperty)
        .to
        .equal("combinator");
      expect(actualValue)
        .to
        .equal("any_combinator_value");
      expect(actualId)
        .to
        .equal("id");
    });
  ...
  describe("#addGroup", () => {
    it("calls the #onGroupAdd from the schema with expected values", () => {
      ...
      expect(actualRuleGroup)
        .to
        .include
        .keys("id", "parentId", "rules");
      expect(actualId)
        .to
        .equal("id");
    });
  ...
  function behavesLikeAnActionElement(label, defaultClassName, customClassName) {
    it("should have the correct label", () => {
      const dom = shallow(<RuleGroup {...props} />);
      expect(dom.find("ActionElement").props().label)
        .to
        .equal(label);
    });
    ...
    it("should have the onClick method handler", () => {
      const dom = shallow(<RuleGroup {...props} />);
      expect(dom.find("ActionElement").props().handleOnClick)
        .to
        .be
        .a("function");
    });
  ...
  function behavesLikeAnElementWithClassNames(element, defaultClassName, customClassName) {
    it("should have the default className", () => {
      const dom = shallow(<RuleGroup {...props} />);
      expect(dom.find(element).props().className)
        .to
        .contain(defaultClassName);
    });
